  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        setError('Произошла неизвестная ошибка при смене телефона');
      }
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <div className="mb-8">
      ...
      {isEditing && !isPhoneRegistered && (
        <AlertMessage
          type="success"
          message="Вы можете изменить номер телефона без подтверждения, так как были
            зарегистрированы по email"
        />
      )}
      ...
      {isEditing && isPhoneRegistered && (
        <AlertMessage
          type="warning"
          message="Для смены номера телефона потребуется подтверждение через СМС."
        />
      )}
