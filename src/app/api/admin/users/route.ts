  ...
  try {
    ...
    const limit = parseInt(
      searchParams.get('limit') ||
        CONFIG.DEFAULT_PAGE_SIZE_USERS_LIST.toString()
    );
    ...
    if (sortBy === 'id') {
      users.sort((a, b) => {
        const decimalA = parseInt(
          getShortDecimalId(a._id.toString())
        );
        const decimalB = parseInt(
          getShortDecimalId(b._id.toString())
        );
